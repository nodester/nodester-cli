...
Nodester.info = function (args) {
  ...
  nodeapi.app_info(appname, function (err, data, original) {
    ...
    var state = false
    if (data.running.hasOwnProperty('indexOf') &&  (data.running.indexOf('error') >-1|| data.running.indexOf('failed-to') > -1)) 
        var state = true;
...
Nodester.restart =function (args) {
  config.check();
  var appname = config.appname;
  if (args.length) {
    appname = args[0].toLowerCase();
...
Nodester.gitreset = function (args) {
  config.check();
  var appname = config.appname;
  if (args.length) {
...
Nodester.init = function (args) {
  ...
  nodeapi.app_info(appname, function (err, data, original) {
    ...
    var child = exec('git clone ' + data.gitrepo + ' ' + folder, function (error, stdout, stderr) {
      ...
      if (isWin){
        var cmd = 'cd ' + folder + ' && git add ' + data.start + ' package.json '
            cmd += '.gitignore && git commit -am "Initial commit via ' + config.brand 
            cmd += '-cli" && git push origin master';
      } else {  
        var cmd = 'cd ' + folder + '; git add ' + data.start + ' package.json '
            cmd += '.gitignore; git commit -am "Initial commit via ' + config.brand 
            cmd += '-cli"; git push origin master;';
